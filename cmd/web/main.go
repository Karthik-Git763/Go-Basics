...
func main() {
	...
	// Define a new CL flag for the mysql DSN string.
	...
	// Close the connection before exiting the main function
	defer db.Close()
	...
	// Initialize the mysql.UserModel instance and add it to the application struct.
	app := &application{
		errorLog:      errorLog,
		infoLog:       infoLog,
		session:       session,
	...
	// We pass in the paths to the TLS certificate and corresponding private key as
	// the two parameters.
